...
public class GenreControllerIT {
	...
	public void findAllShouldReturnAllGenresWhenVisitorAuthenticated() throws Exception {
		...
		result.andExpect(jsonPath("$[1].id").value(3L));
		result.andExpect(jsonPath("$[1].name").value("Drama"));
		result.andExpect(jsonPath("$[2].id").value(2L));
		result.andExpect(jsonPath("$[2].name").value("Terror"));
